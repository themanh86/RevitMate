...
## ⚠️ .NET 8 + Revit 2026 に関する重要な注意点

.NET Framework 4.8 から .NET 8 へ移行する際の主な相違点:

| 項目 | .NET Framework 4.8 | .NET 8(採用) |
|---|---|---|
| プロジェクト SDK | 旧形式 `<Project>` | `Microsoft.NET.Sdk`(SDK-style) |
| TargetFramework | `net48` | `net8.0-windows`(WPF 利用のため `-windows` が必要) |
| `UseWPF` | 暗黙的 | `<UseWPF>true</UseWPF>` の明示的指定が必要 |
| 設定ファイル | `app.config` / `App.config` | `appsettings.json` |
| `System.Configuration` | 標準搭載 | `Microsoft.Extensions.Configuration.*` が必要 |
| `ProtectedData`(DPAPI) | 標準搭載 | NuGet `System.Security.Cryptography.ProtectedData` が必要 |
| リソース(.resx) | `ResXFileCodeGenerator` | 利用可能だがジェネレーターが若干異なる |
| HttpClient | 利用可能 | 利用可能(性能向上) |
| Newtonsoft.Json | 一般的 | 利用可能、または `System.Text.Json` 標準搭載 |

**Revit 2026 の Autodesk 公式情報**: Revit 2026 SDK は .NET 8 を前提に出荷されています。`C:\Program Files\Autodesk\Revit 2026\` から `RevitAPI.dll` / `RevitAPIUI.dll` を参照する際は、必ず `net8.0-windows` をターゲットにしてください。

**推奨事項**:
- 4 プロジェクトすべて `net8.0-windows` をターゲットとする(WPF + i18n + Revit 連携のため)
- 新規シリアライズには `System.Text.Json`(軽量・高速)を使用、もしくは Claude API DTO のマッピングが容易な場合は `Newtonsoft.Json` を維持
- 設定は `appsettings.json` + `Microsoft.Extensions.Configuration` で管理
...
4. **Visual Studio 2022 のインストール**
   - ワークロード:「.NET デスクトップ開発」
   - Revit SDK 2026 をインストール(Autodesk Developer Network より)
