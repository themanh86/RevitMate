...
## ⚠️ Lưu ý quan trọng về .NET 8 + Revit 2026

Khi chuyển từ .NET Framework 4.8 sang .NET 8, có một số điểm khác biệt cần biết:

| Khía cạnh | .NET Framework 4.8 | .NET 8 (chọn) |
|---|---|---|
| Project SDK | `<Project>` cũ | `Microsoft.NET.Sdk` (SDK-style) |
| TargetFramework | `net48` | `net8.0-windows` (cần `-windows` cho WPF) |
| `UseWPF` | implicit | phải set `<UseWPF>true</UseWPF>` |
| Config file | `app.config` / `App.config` | `appsettings.json` |
| `System.Configuration` | có sẵn | cần `Microsoft.Extensions.Configuration.*` |
| `ProtectedData` (DPAPI) | có sẵn | cần NuGet `System.Security.Cryptography.ProtectedData` |
| Resources (.resx) | `ResXFileCodeGenerator` | vẫn dùng được nhưng generator hơi khác |
| HttpClient | có | có (tốt hơn) |
| Newtonsoft.Json | dùng | dùng được, hoặc dùng `System.Text.Json` built-in |

**Tài liệu Autodesk cho Revit 2026**: SDK của Revit 2026 ship với .NET 8. Khi reference `RevitAPI.dll`/`RevitAPIUI.dll` từ `C:\Program Files\Autodesk\Revit 2026\`, đảm bảo project target `net8.0-windows`.

**Khuyến nghị**:
- Toàn bộ 4 project target `net8.0-windows` (vì có WPF + i18n + interop với Revit)
- Dùng `System.Text.Json` cho serialization mới (gọn, hiệu năng cao) hoặc giữ `Newtonsoft.Json` nếu Claude API DTOs dễ map hơn
- Config qua `appsettings.json` + `Microsoft.Extensions.Configuration`
...
4. **Cài Visual Studio 2022**
   ...
   - Cài Revit SDK 2026 (Autodesk Developer Network)
